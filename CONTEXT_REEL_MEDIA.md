...
- **⚠️ CORRECTIF SUIVANT, isolé (8170 → 8172) : LE CACHE DU FICHIER HISTORIQUE.** Découvert en production : après la régénération, Netlify servait le bon fichier (vérifié à l'URL directe, M651 absent) mais **l'app affichait encore les doublons**. Seule la navigation privée montrait le bon résultat. Le fetch utilisait `cache:'force-cache'` — « utilise le cache **sans revalider** ».
  **Toute l'équipe aurait gardé une version périmée après chaque régénération, sans comprendre pourquoi.**
  **⚠️ `no-cache` RETENU CONTRE le paramètre de version que proposait le Pilote.** Argument de Claude Code, décisif : `?v=2026-09-24` aurait marché, mais **repose sur un geste humain** (incrémenter à chaque régénération) et coupleraient deux fichiers — exactement la classe de bug qui venait de mordre. `no-cache` = revalidation conditionnelle → **304 si inchangé** (quasi gratuit), 200 sinon. **Le fix supprime l'étape humaine au lieu de la déplacer.**
- **Vérification Pilote :** `wc -l` 8170 puis **8172**, compteurs 2/32/4-4, `node --check` OK, `force-cache` restant = 0. Surlignage **exécuté** sur 4 cas d'accents et 4 tentatives d'injection. `filtreHistorique` utilise `_norm`, `applySearch` normalise à la volée, la recherche de carte du formulaire Tâches passe toujours par `applySearch`. HAVANA, `corrigerTousRetours`, `PAD_FOLDER_URL` intacts.
...
- **⚠️ CORRECTIF IMMÉDIAT — doublon de bouton (7636 → 7634).** Deux boutons « + Nouvelle tâche » s'affichaient : celui de l'en-tête de page (`main-actions`) et un second ajouté dans `renderTachesPerso`. Repéré par David en production dès le premier chargement. Le second retiré, l'en-tête conservé. Chasse aux autres résidus de l'ancien rendu faite : aucun (pills de stats, toggle « Afficher terminées », case à cocher, modale d'édition, texte « Vos rappels personnels » — tous absents).
- **Vérification Pilote :** `wc -l` **7634**, `CHEF_PAR_DEFAUT`=12, `createNotif` **24 → 28** (les 2 nouveaux types, légitime), script 4/4, `node --check` OK. `filtreTaches` = 3 occurrences, `is_empty` = 1 (un commentaire). Un seul bouton réel. `stopPropagation` confirmé. Les 5 orphelins à 0. `fermerSurClicFond` **12 → 11** (la modale « modifier une tâche » supprimée — cohérent). Non-régression vérifiée.
...
- **LE CAS RÉEL :** carte de Camille, **V4 déposée par David**, le champ « Responsable » affichait **Camille**. David n'a reçu **aucune notification** alors qu'il devait s'occuper de la suite.
- **LA CAUSE :** `creerVersion` initialisait `Monteur = s.journaliste` (le journaliste de la carte), et le dépôt du lien ne corrigeait jamais cette valeur. Seul un **sélecteur manuel** permettait de la rectifier — que personne n'utilisait.
- **⚠️ AUCUNE NOTIFICATION NE CIBLAIT LE MONTEUR.** Retour, validation, annulation : tout partait au `journaliste` de la carte. Celui qui avait monté ne savait pas qu'on lui demandait des corrections.
- **⚠️⚠️ DÉCOUVERTE QUI A CHANGÉ LA CONCEPTION : `review.html` notifiait DÉJÀ le monteur** (Set `{chef, journaliste, monteur de la version, contactBrand}` − auteur). Mais il lit ce monteur dans les champs **carte `Monteur V1/V2/V3`**, que l'app **n'écrivait jamais** → le mécanisme était **affamé depuis toujours**. Le champ n'était pas mort : il était **consommé sans être alimenté**.
- **STRUCTURE : rien à créer.** `DB_VERSIONS` porte un `Monteur` **par ligne version** → l'historique « Camille a fait V1-V2, David V3 » était déjà stockable. Le problème n'était pas la structure, c'était **la valeur**.
- **Mesuré :** 384 versions réelles sur 170 cartes ; **11 cartes portent au moins 2 monteurs distincts** — corrections manuelles faites au sélecteur. Exemple : Camille sur V1-V3/V5/V6, **Mickaël sur V4**. Le compte exact `Monteur ≠ Journaliste` exigeait une jointure multi-bases, **bloquée par le plan Notion** — non nécessaire pour décider.
- **RÈGLE 1 — le déposant devient monteur.** `creerVersion` écrit `currentUser.nom`. **Aucune réécriture rétroactive** : les 384 versions gardent leur valeur, seules les nouvelles prennent le déposant.
- **RÈGLE 2 — retour, validation et annulation notifient le journaliste ET le monteur de la version concernée.** L'annulation notifie désormais le monteur de la version annulée — c'est lui qui doit refaire.
- **⚠️ DÉDUP PAR SET, pas par la fenêtre serveur.** `notifierVersion` construit `Set{journaliste, monteur, (chef si demandé)} − auteur`, un envoi par nom restant. Le TTL de 20 s de `notify.js` est un **filet, pas une garantie** — une personne cumulant deux rôles reçoit **une** notification.
- **⚠️ B1 RETENU — source de vérité = `DB_VERSIONS`, +1 lecture par envoi.** L'alternative (lire le miroir carte, 0 requête) **plafonnait à V1-V3** — or ce sont les cartes très retravaillées (des V8 existent) où le monteur compte le plus. Résolution par **`versionId` exact** pour validation/annulation (des cartes de test ont des numéros dupliqués), par `(sujetId, numéro)` pour les retours.
- **⚠️ FONCTION D'ÉCRITURE UNIQUE `ecrireMonteurVersion` — la réserve du Pilote.** Deux champs pour une même information, c'est une **divergence en attente**. Une règle documentée ne suffit pas : elle doit être **impossible à contourner**. D'où une fonction unique appelée par les **deux** sites (`creerVersion` et `updateVersionMonteur`), et le `Monteur` retiré du POST de création.
- **ÉCHEC PARTIEL — l'ordre est délibéré :** (1) `DB_VERSIONS.Monteur` **en premier, obligatoire** — si elle échoue, `throw`, rien d'autre n'est tenté ; (2) miroir carte `Monteur V{n}` **ensuite, best-effort** en `try/catch` — son échec ne fausse rien, les notifs lisent la source de vérité. **Le miroir ne peut que retarder, jamais fausser.**
- **Le miroir carte sert TROIS consommateurs :** `review.html` (enfin alimenté), `mesCartes` (qui référence déjà `monteur1/2/3` → « mes cartes » s'allume **sans toucher au prédicat**), et « activité récente » via les notifs élargies. La mémoire locale `s.monteur{n}` est alignée → pas de rechargement nécessaire.
- **⚠️ LIMITE V4+ RENDUE VISIBLE** — la carte n'a que 3 slots, donc `review.html` ne notifiera pas le monteur d'une V4 et au-delà. Limite **préexistante** de review.html, mais maintenant qu'on alimente le champ, quelqu'un croirait que ça marche partout. **Commentée à l'endroit exact de l'écriture.** L'app, elle, reste correcte pour toute version (elle lit `DB_VERSIONS`).
- **Sélecteur « Responsable » manuel CONSERVÉ** (corriger un mauvais déposant), mais **routé par la fonction unique** → il écrit désormais les deux champs.
- **`onLienBlur` / Lien V1-V3 : chemin MORT** — `onLienBlur` n'est câblé nulle part dans l'interface (seuls `lien1/2/3` sont encore lus pour l'affichage et la recherche). Laissé tel quel.
- **`review.html` n'a RIEN à changer** — il consomme déjà le champ avec la même dédup par Set.
- **⚠️ COMPTEURS DE PRÉSERVATION MODIFIÉS, et c'est LÉGITIME :** `CHEF_PAR_DEFAUT` **13 → 12**, `createNotif` **29 → 24**. Les 4 sites de notification appelaient chacun `createNotif` en direct avec leur propre fallback de chef ; ils passent par `notifierVersion` qui centralise. Vérifié site par site. **Nouveaux compteurs de référence : 12 et 24.**
- **Vérification Pilote :** `wc -l` 7556, script 4/4, `node --check` OK sur les 2 blocs séparément. Fonction unique et ordre d'écriture confirmés par lecture. Le retour porte bien **sa** version (`ret-version`), pas la dernière. Porte anti-rafale préservée aux deux sites. Non-régression (`escJs`, `texteLibre`, `urlSure`, `fermerSurClicFond`, `matchFiltre`) vérifiée.

### 2026-09-02 — Formatage des textes libres + href sécurisé (texteLibre / urlSure)
- **`index.html` seul, 7470 → 7496 (+26).**
- **LE SYMPTÔME (retour de Louise) :** un texte structuré collé depuis Slack — paragraphes, puces, lien — s'affichait dans l'app comme **un bloc compact illisible**. Louise ajoutait le lien du document à côté, probablement parce qu'elle savait que le commentaire rendait mal.
- **⚠️ LA DONNÉE ÉTAIT INTACTE — c'est l'affichage qui l'aplatissait.** Mesuré : **17 commentaires sur 74 contiennent encore des `\n`** en base. Donc les anciens commentaires **retrouvent leur mise en forme rétroactivement**, sans migration.
- **DEUX DÉFAUTS SUR LA MÊME LIGNE :** pas de `white-space:pre-line` (les `\n` réduits à une espace par le HTML — le bug visible) **ET** `${texte}` inséré dans `innerHTML` **sans échappement** → **faille d'injection réelle**. `pre-line` seul ne l'aurait pas fermée.
- **⚠️ LE RECENSEMENT LARGE A TOUT CHANGÉ.** Le Pilote a exigé de « chercher large » après l'expérience de `pointerdown` (7 sites annoncés, 12 réels). Résultat : le schéma `texte Notion → innerHTML non échappé` est **quasiment toute la couche de rendu**. Deux facettes séparées : **2a** multi-ligne (perte visible + injection) et **2b** champs courts (injection seule).
- **Détections qui ont évité un correctif partiel :** la description, la **reformulation** et la **raison d'impossibilité** d'un retour vivent dans le **même littéral de template** — corriger l'une sans les autres aurait laissé le bug. Et le « **Brief** » n'est pas un champ distinct : c'est **la Notes chef**.
- **HELPER `texteLibre(s)` (l.587)** — `<span style="white-space:pre-line">` + `escapeHtml(s)`. **Les deux préoccupations en un seul geste, indissociables** : impossible d'échapper sans `pre-line` ou l'inverse. Le Pilote a refusé l'alternative « `pre-line` sur le conteneur + `escapeHtml` dans le contenu » — deux gestes séparés, c'est un geste oubliable.
- **⚠️ NE PAS utiliser `texteLibre` dans une `<textarea>`** : contexte texte brut, une balise s'y afficherait littéralement. Y employer `escapeHtml` **seul** — la textarea gère les `\n` nativement, et l'échappement y reste **nécessaire** (il neutralise un `</textarea>` qui permettrait un break-out).
- **⚠️⚠️ `urlSure(u)` — LE POINT QUE `escapeHtml` NE POUVAIT PAS RÉGLER.** Les liens de référence étaient rendus dans un `href` avec l'URL brute : **`javascript:alert(1)` ne contient aucun caractère échappé** et restait exécutable au clic. C'est un vecteur d'exécution direct, pas une injection de balise — Claude Code l'avait classé « à traiter à froid », le Pilote l'a rapatrié dans ce chantier.
  Validation de **schéma** : seuls `http://` et `https://` passent. Testé par le Pilote sur 12 vecteurs — **bloqués** : `javascript:`, `JavaScript:` (casse mixte), précédé d'espaces ou d'une tabulation, `data:text/html`, `vbscript:`, `//evil.com`, `https:/x.fr` (une seule barre). **Passés** : `http(s)://` et `HTTPS://`.
  Cas limite vérifié : `https://x.fr" onmouseover="alert(1)` **passe** `urlSure` (schéma valide) mais `escapeHtml` transforme les guillemets en `&quot;` → **pas de sortie d'attribut**. Les deux protections se complètent.
- **Repli propre** : schéma refusé → **plus de balise `<a>` du tout**, un `<span>` non cliquable « ⚠ lien invalide ». Le vecteur disparaît, l'information reste visible.
- **8 sites traités** : commentaire, retour (desc + reformulation + reformulé par + raison impossible, en liste **et** dans le player), notes chef (textarea, `escapeHtml` seul), idée description (teaser en div → `texteLibre`, modale en textarea → `escapeHtml`).
- **⚠️ FACETTE 2b RESTANTE, POUR MÉMOIRE** — vraies failles d'injection du même type, `escapeHtml` seul suffira, **chantier sécurité dédié à faire un jour** : titres de sujets (l.798, 5507, 5539, 7423, ~921), titres/proposé par/raison de déclin des idées, titres de tâches, musiques (titre/artiste/durée), notifications (message + titre du sujet), **contacts prénom/nom en contexte `value="…"`** (un `"` casse l'attribut), et les liens de référence en brouillon (rendus en `<span>`, sans vecteur `javascript:`).
- **Vérification Pilote :** `wc -l` 7496, compteurs intacts, `node --check` OK sur les **2 blocs `<script>` vérifiés séparément** (fidèle à la façon dont le navigateur les parse). `urlSure` **ré-exécuté indépendamment sur 12 vecteurs**. Aucun rendu brut résiduel sur les champs traités. Non-régression (`escJs`, `fermerSurClicFond`, `matchFiltre`, `_createEnCours`) vérifiée.
...
- **Le problème :** le PAD était un cul-de-sac — **234 cartes sur 318**, dont 179 dormant depuis plus d'un mois. Tous les mécanismes de sortie étaient morts (Date de diffusion 1/318, Archivé 0/318). Personne ne revient remplir un champ après coup.
- **Cron `0 3 * * *`** (chaque nuit à 3h), **plafond 80 cartes par passe et par nuit** → le fonds de 217 se draine en ~3 nuits. Coût : **~30 invocations Netlify/mois**, négligeable face au quota (94K/125K).
- **PASSE 1 — RATTRAPAGE :** les cartes `Statut=PAD` + `Archivé=false` + `Date de diffusion is_empty` + **`Format ≠ Brand`** reçoivent `Date de diffusion = last_edited_time`. Règle de David : la dernière modification d'une carte correspond en général à la fin de son cycle (dépôt du lien final, passage en PAD) — approximation acceptable sur le fonds ancien.
- **PASSE 2 — PERMANENTE :** les cartes `Statut=PAD` + `Archivé=false` + `Date de diffusion on_or_before (now−30j)` reçoivent `Archivé=true`. `on_or_before` **exclut nativement les dates vides** → on n'archive jamais une carte sans date de diffusion (une carte en stock attend légitimement).
- **⚠️⚠️ LE PIÈGE TEMPOREL — LA LEÇON CENTRALE DE CE CHANTIER.** Écrire `Date de diffusion` met automatiquement `last_edited_time` à **aujourd'hui**. Exemple de David : *« on lit 20 juillet, on écrit 20 juillet, mais dès qu'on écrit la carte devient modifiée aujourd'hui — il faut éviter de rentrer dans un loup temporel »*. Une **seule passe de trop** aurait daté les 217 cartes au 26 août et détruit définitivement les vraies dates.
- **NEUTRALISÉ PAR CONSTRUCTION, pas par timing :** (1) la **passe 1 est le SEUL code qui écrit `Date de diffusion`** — la passe 2 ne l'utilise qu'en **filtre de lecture** (l.116) et n'écrit que `Archivé` (l.122) ; (2) la passe 1 ne sélectionne que `date is_empty` (l.84) → dès qu'une carte est datée elle **quitte définitivement** cet ensemble ; (3) `last_edited_time` est **lu depuis le résultat de la requête** (l.89), avant toute écriture. Ce n'est pas « la 2ᵉ passe tourne après la 1ʳᵉ » (probabiliste), c'est **« aucune passe ne peut réécrire une diffusion existante »** (structurel). Crash au milieu et double-run sont **idempotents**.
- **⚠️ `Statut = PAD` SUR LA PASSE 2 — arbitrage du Pilote contre la reco de Claude Code.** Il proposait la passe permanente **sans** filtre Statut (« en pratique seules les ex-PAD portent une date de diffusion »). Refusé : le champ `Date de diffusion` est **saisissable à la main** dans la fiche — quelqu'un pourrait y noter une date prévisionnelle sur une carte en montage, qui serait alors **archivée en pleine production**. Une clause coûte moins qu'un usage supposé.
- **⚠️ LE BRAND EST EXCLU DU RATTRAPAGE** : un client Brand peut valider puis programmer des semaines plus tard — `last_edited_time` y serait un **faux proxy**. Ces cartes restent en Production jusqu'à recevoir une vraie date via le popup (chantier suivant). La passe permanente, elle, s'applique à tout : une Brand **avec** une date dépassée est bien terminée.
- **Découverte qui a débloqué le chantier :** `last_edited_time` est **lisible directement dans le résultat de `databases/{id}/query`** (l'app le fait déjà, `parseSujet` l.731). Master croyait devoir fetcher page par page — **~3 lectures au lieu de 217**. C'est ce qui rend l'opération faisable en une fonction plutôt qu'en plusieurs sessions MCP.
- **Pourquoi une Scheduled Function et pas Master :** Master avait chiffré ~434 opérations avec un rate limit MCP tombant toutes les 10-15 requêtes → plusieurs sessions, risque à chaque reprise. La fonction parle à Notion **directement** (`NOTION_TOKEN`, hors proxy), sans cette contrainte. **On construit le mécanisme, et le mécanisme fait le ménage** — plutôt que de faire le ménage à la main puis construire le mécanisme.
- **Patron `drive-permsweep` réutilisé :** `summary` de compteurs, `try/catch` par carte (`erreurs++`), **logs = compteurs uniquement** (jamais titre ni code), arrêt propre sur **429** (`rateLimited=true; break`) → reprise la nuit suivante. Espacement 350 ms entre écritures (~3 req/s).
- **`queryAll` collecte TOUTES les pages AVANT d'écrire** — muter l'ensemble filtré pendant la pagination ferait dériver le curseur.
- **Stats non faussées :** les dates rétroactives sont anciennes (hors mois courant) et les cartes archivées quittent `sujets` → « Ton mois · PAD livrés » et les compteurs sidebar sont neutres. `cnt-archives` grossit (correct). *Si* on voulait que ces dates peuplent l'historique « PAD par mois », il faudrait que ces stats incluent les archivées — **chantier séparé, non retenu**.
- **⚠️ CE CHANTIER NE FAIT QUE LA MOITIÉ DE L'APRÈS-PAD.** Restent à faire dans `index.html` : le **popup au moment du PAD** (« diffusée le / dans la semaine / garder en stock »), la **section Diffusion** sur la carte, le bouton **« Marquer comme diffusée »**, le **bloc archivage sorti du PAD**, et les **filtres Stock / Diffusée** dans la barre (en retirant les formats, déjà présents en sidebar — le filtre est **exclusif**, donc aucun croisement n'est perdu).
- **Entre les deux chantiers :** une nouvelle carte passée en PAD sans date sera rattrapée par la passe 1 (date ≈ jour du PAD) puis archivée 30 j après. Automatique, sans intervention. Le popup n'activera pas le mécanisme — **il l'alimentera plus précisément** et permettra le **stock**.
- **Vérification Pilote :** `node --check` OK. **Deux corps de PATCH seulement** dans tout le fichier — l.94 (`Date de diffusion`, passe 1) et l.122 (`Archivé`, passe 2). `last_edited_time` lu l.89 depuis le résultat. `queryAll` sans écriture pendant la pagination. `netlify.toml` conserve bien `drive-permsweep`.
- **Rollback :** supprimer `pad-sweep.js` et son bloc `netlify.toml` = comportement précédent. Les cartes archivées se restaurent une par une depuis la vue Archives.
